...
const Videos = () => {
  ...
  return <RoleGuard allowedRoles={['student', 'admin', 'mentor', 'superadmin']}>
      <div className="space-y-6 animate-fade-in">
        ...
        {modules.length === 0 ? <Card className="shadow-medium border-border/50">
            ...
            {modules.map(module => <Card key={module.id} className="shadow-lg border-0 bg-gradient-to-br from-white to-gray-50">
                  ...
                  <CollapsibleTrigger asChild>
                    <CardHeader className="cursor-pointer hover:bg-gray-50/50 transition-colors border-b">
                      <div className="flex items-center justify-between">
                        ...
                        {expandedModules.has(module.id) ? <ChevronDown className="w-5 h-5 text-muted-foreground" /> : <ChevronRight className="w-5 h-5 text-muted-foreground" />}
                      </div>
                    </CardHeader>
                  </CollapsibleTrigger>
                  ...
                  <CollapsibleContent>
                    <CardContent className="p-0">
                      <div className="space-y-2 p-4">
                        {module.recordings.map((recording, index) => <div key={recording.id} className={`flex items-center justify-between p-4 rounded-lg border transition-all ${recording.isUnlocked && userLMSStatus === 'active' ? 'bg-white border-border hover:border-primary/30 hover:shadow-sm' : 'bg-muted/30 border-muted'}`}>
                            <div className="flex items-center gap-4">
                              <div className="flex items-center justify-center w-8 h-8 rounded-full bg-muted text-sm font-medium">
                                {index + 1}
                              </div>
                              
                              {recording.isUnlocked && userLMSStatus === 'active' ? recording.isWatched ? <CheckCircle className="w-5 h-5 text-success" /> : <Play className="w-5 h-5 text-primary" /> : <Lock className="w-5 h-5 text-muted-foreground" />}
                              ...
                              <div className="flex-1">
                                ...
                                <div className="flex items-center gap-3 text-sm text-muted-foreground mt-1">
                                  {recording.duration_min && <span className="flex items-center gap-1">
                      ...
                      {recording.isWatched && <Badge variant="outline" className="bg-[hsl(var(--success))] text-[hsl(var(--success-foreground))] border-[hsl(var(--success))] bg-green-600">
    <CheckCircle className="w-3 h-3 mr-1" />
    Completed
  </Badge>}
                              
                              {recording.hasAssignment && recording.assignmentSubmitted && <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">
                              ...
                              <Button variant={recording.isWatched ? "outline" : "default"} size="sm" disabled={userLMSStatus !== 'active' || !recording.isUnlocked || !recording.recording_url} onClick={() => handleWatchRecording(recording)} className={!(recording.isUnlocked && userLMSStatus === 'active') ? 'opacity-50' : ''}>
                                ...
                                {userLMSStatus !== 'active' ? 'Clear Fees' : recording.isUnlocked ? recording.isWatched ? 'Rewatch' : 'Watch Now' : 'Locked'}
                              </Button>
                            </div>
                          </div>)}
                      </div>
        ...
        {selectedRecording && selectedRecording.hasAssignment && <StudentSubmissionDialog open={assignmentDialogOpen} onOpenChange={open => {
      ...
      }} userId={user?.id || ""} hasSubmitted={selectedRecording.assignmentSubmitted} onSubmissionComplete={() => {
        setTimeout(refreshData, 500);
      }} />}
      </div>
    </RoleGuard>;
