-- Phase 1: Complete Database Schema Redesign (Fixed interval constraint)
-- Drop existing user-related tables and functions to start fresh

-- Drop existing triggers first (including the one causing the issue)
...
DROP TRIGGER IF EXISTS trigger_generate_student_id ON users;

-- Drop existing functions with CASCADE to handle dependencies
DROP FUNCTION IF EXISTS public.create_student_atomic(uuid, text, text, text, integer, uuid, uuid) CASCADE;
DROP FUNCTION IF EXISTS public.create_student_atomic(text, text, text, integer, uuid, uuid) CASCADE;
DROP FUNCTION IF EXISTS public.create_student_complete(text, text, text, text, text, uuid, uuid, uuid) CASCADE;
DROP FUNCTION IF EXISTS public.delete_student_atomic(uuid) CASCADE;
DROP FUNCTION IF EXISTS public.generate_student_id() CASCADE;
DROP FUNCTION IF EXISTS public.notify_admins_user_created() CASCADE;
DROP FUNCTION IF EXISTS public.enqueue_student_onboarding_jobs(uuid) CASCADE;
DROP FUNCTION IF EXISTS public.initialize_student_unlocks(uuid) CASCADE;
...
CREATE TABLE installment_plans (
  ...
  interval_days INTEGER NOT NULL CHECK (interval_days >= 0), -- Allow 0 for single payment
