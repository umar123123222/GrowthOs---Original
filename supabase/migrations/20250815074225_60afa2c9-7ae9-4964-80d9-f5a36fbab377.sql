-- Fix critical security vulnerabilities and add missing database components (fixed version)

-- 1. Fix company_settings RLS policies (replace existing with proper security)
DROP POLICY IF EXISTS "Staff can view company settings" ON public.company_settings;
DROP POLICY IF EXISTS "Students can read contact email for support" ON public.company_settings;
...
-- 2. Fix success_sessions RLS policies (replace existing with proper authentication)
