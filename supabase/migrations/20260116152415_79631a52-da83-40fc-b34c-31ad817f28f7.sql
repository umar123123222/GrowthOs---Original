-- Drop and recreate the function to fix the ordering and duplicates issue
...
BEGIN
  ...
  -- Get current step number from pathway_courses directly
  SELECT pc.step_number INTO v_current_step
  FROM pathway_courses pc
  WHERE pc.pathway_id = p_pathway_id 
    AND pc.course_id = v_current_course_id;

  -- Return the pathway courses in correct order
  RETURN QUERY
  WITH choice_selections AS (
  ...
  SELECT 
    ...
    -- Available if: step <= current step, or is the current course
    (pc.step_number <= COALESCE(v_current_step, 1) OR pc.course_id = v_current_course_id)::boolean as is_available,
    -- Completed check
    public.is_course_fully_completed(p_user_id, pc.course_id)::boolean as is_completed,
    -- Is current course
    ...
    (COALESCE(pc.is_choice_point, false) = true AND pc.choice_group IS NOT NULL AND 
    ...
    CASE 
      WHEN COALESCE(pc.is_choice_point, false) = true AND pc.choice_group IS NOT NULL THEN
  ...
  WHERE pc.pathway_id = p_pathway_id
  ORDER BY pc.step_number ASC, pc.choice_group NULLS FIRST, c.title ASC;
END;
$$;