...
BEGIN
  ...
  -- Detect full bypass on ANY active enrollment that grants this course
  -- (direct course enrollment OR pathway enrollment that contains this course).
  SELECT true
  INTO v_full_bypass
  ...
  WHERE ce.student_id = v_student_id
    ...
    AND (
      ...
      OR EXISTS (
        ...
        WHERE pc.pathway_id = ce.pathway_id AND pc.course_id = p_course_id
      )
    )
  LIMIT 1;
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE (ce.course_id = c.id
           OR EXISTS (SELECT 1 FROM pathway_courses pc WHERE pc.pathway_id = ce.pathway_id AND pc.course_id = c.id))
      AND ce.student_id = v_student_id
      AND ce.status = 'active'
    ORDER BY
      (ce.drip_override = true AND COALESCE(ce.drip_enabled, false) = false
        AND ce.sequential_override = true AND COALESCE(ce.sequential_enabled, false) = false) DESC,
      (ce.batch_id IS NOT NULL) DESC,
      (ce.pathway_id IS NOT NULL) DESC,
      ce.enrolled_at DESC NULLS LAST,
      ce.created_at DESC NULLS LAST
  ...
  -- Apply full bypass: clear all gates.
  ...
  SELECT
    ...
    CASE WHEN v_full_bypass THEN NULL ELSE fs.calculated_drip_date END as drip_unlock_date
  FROM full_status fs
  ORDER BY fs.seq_pos;
END;
$function$;