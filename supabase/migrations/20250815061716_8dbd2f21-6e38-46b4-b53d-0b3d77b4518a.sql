...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_submissions_student_id ON public.submissions(student_id);

-- Index for notifications by user and status (notification queries)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_notifications_user_status ON public.notifications(user_id, status);

-- Index for available_lessons by sequence_order (lesson ordering)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_available_lessons_sequence ON public.available_lessons(sequence_order);

-- Index for invoices by student_id and status (financial tracking)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_invoices_student_status ON public.invoices(student_id, status);

-- Index for user_activity_logs by user_id and created_at (activity tracking)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_activity_logs_user_date ON public.user_activity_logs(user_id, created_at DESC);

-- Index for admin_logs by entity_type and created_at (admin audit trails)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_admin_logs_entity_date ON public.admin_logs(entity_type, created_at DESC);