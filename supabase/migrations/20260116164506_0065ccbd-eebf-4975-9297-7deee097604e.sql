-- Update the sequential unlock RPC to use batch start_date for drip calculation when batch_id is set
-- If no batch is assigned, fall back to student's enrollment_date (LMS access date)
...
BEGIN
  ...
  -- Get batch_id from enrollment if exists
  SELECT ce.batch_id
  INTO v_batch_id
...
-- Add comment explaining the batch-aware drip calculation
COMMENT ON FUNCTION public.get_course_sequential_unlock_status(uuid, uuid) IS 'Returns sequential unlock status for course recordings. Drip calculation uses batch start_date if student is assigned to a batch, otherwise falls back to student enrollment_date.';