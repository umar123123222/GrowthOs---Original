...
BEGIN
  ...
  SELECT
    ...
    (
      -- Priority order:
      -- 1. Manual admin unlock always wins
      -- 2. Already watched videos stay unlocked (grandfathered) — drip cannot re-lock them
      -- 3. Drip lock for upcoming videos (not yet watched)
      -- 4. Course access / fees gates
      -- 5. Sequential rules
      CASE
