...
CREATE POLICY "Everyone can view milestone categories" ON public.milestone_categories FOR SELECT TO authenticated USING (true);
CREATE POLICY "Admins can manage milestone categories" ON public.milestone_categories FOR ALL TO authenticated 
  USING (get_current_user_role() = ANY(ARRAY['admin', 'superadmin']));

-- RLS Policies for milestones
CREATE POLICY "Everyone can view milestones" ON public.milestones FOR SELECT TO authenticated USING (is_active = true);
CREATE POLICY "Admins can manage milestones" ON public.milestones FOR ALL TO authenticated 
  USING (get_current_user_role() = ANY(ARRAY['admin', 'superadmin']));

-- RLS Policies for user_milestones
CREATE POLICY "Users can view their own milestones" ON public.user_milestones FOR SELECT TO authenticated 
  USING (auth.uid() = user_id);
CREATE POLICY "Staff can view all user milestones" ON public.user_milestones FOR SELECT TO authenticated 
  USING (get_current_user_role() = ANY(ARRAY['admin', 'superadmin', 'mentor', 'enrollment_manager']));
CREATE POLICY "System can award milestones" ON public.user_milestones FOR INSERT TO authenticated 
  USING (true);
CREATE POLICY "Admins can manage user milestones" ON public.user_milestones FOR ALL TO authenticated 
  USING (get_current_user_role() = ANY(ARRAY['admin', 'superadmin']));
