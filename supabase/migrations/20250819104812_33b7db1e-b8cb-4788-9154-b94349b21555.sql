-- PHASE 2: CRITICAL SECURITY FIX - Remove SECURITY DEFINER Views
...
-- Step 1: Backup existing view data before dropping
CREATE TABLE IF NOT EXISTS user_security_summary_backup AS 
SELECT * FROM user_security_summary;

CREATE TABLE IF NOT EXISTS segmented_weekly_success_sessions_backup AS 
SELECT * FROM segmented_weekly_success_sessions;
...
-- Mentors can view assigned students only (this will need refinement based on actual mentor-student relationships)
...
-- Step 7: Create function to sync user_security_summary from users table
CREATE OR REPLACE FUNCTION sync_user_security_summary()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
...
BEGIN
  -- Clear existing data
  DELETE FROM user_security_summary;
...
-- Step 8: Create function to sync segmented_weekly_success_sessions
CREATE OR REPLACE FUNCTION sync_segmented_weekly_success_sessions()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
...
BEGIN
  -- Clear existing data
  DELETE FROM segmented_weekly_success_sessions;
  
  -- Populate with current success session data
...
SELECT sync_user_security_summary();
SELECT sync_segmented_weekly_success_sessions();

-- Step 10: Create triggers to keep data in sync
